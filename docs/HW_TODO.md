...
- `[x] VERIFIED 2026-09-13`: A fresh Revocation typed-data signature was approved on the physical Ledger through the DMK Node HID flow; the broker relay confirmed registry status `REVOKED` and submitted the HCS `REVOKED` envelope.
...
- `[x] VERIFIED 2026-09-13`: `/finity revoke` signing a Revocation on a physical device and confirming the broker-relayed transaction reaches registry status `REVOKED` plus an HCS `REVOKED` envelope. The narrow daemon route and failure behavior are unit-verified.
- `HW-UNVERIFIED`: A physical mandate run reached Ledger signature completion but failed during broker registration with the previously generic `invalid_request`; retry the updated local signature verification and safe error path on the connected device.
